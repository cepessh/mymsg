...
private:
  ...
  struct LogonState {
    ...
    static bool format_is_valid(const std::string& login) noexcept {
  ...
  void _enter_login() {
    post(ui_strand, [this]() mutable {
      while (std::cin >> logonState.login &&
             !logonState.format_is_valid(logonState.login)) {
  ...
  void onPasswordValidityResponseReceived(error_code ec) {
    ...
    case AUTHORIZED: {
      logger->set_pattern("[%d/%m/%Y] [%H:%M:%S:%f] [%n] %^[%l]%$ [" + logonState.login + "] %v");
      clear_console();
  ...
  void onActionRequestReceived(error_code ec) {
    ...
    post(ui_strand, [this] {
      ActionChoice action_choice;
      {
        ...
        action_choice = static_cast<ActionChoice>(std::stoi(action_choice_temp));
      }
      
      /// Sends action choice
      m_outbuf = std::to_string(action_choice) + "\n";
      asio::async_write(m_sock, asio::buffer(m_outbuf), 
        [this, action_choice] (error_code ec, std::size_t /*bytes_transferred*/) {
  ...
  void onChatRequestSent(error_code ec) {
    ...
    asio::async_read_until(m_sock, m_inbuf, "\n\n", 
      [this] (error_code ec, std::size_t /*bytes_transferred*/) {
        onCharResponseReceived(ec);
      });
  } 

  void onCharResponseReceived(error_code ec) {
